      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="bg-white p-6 rounded-[2rem] border-2 border-blue-500 shadow-lg sticky top-20 z-40 space-y-4">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            <h3 className="text-xl font-black text-slate-900 flex items-center gap-2 shrink-0">
              <span className="bg-blue-500 text-white p-1.5 rounded-lg text-xs">BATCH</span>
              여러 부스 일괄 관리
            </h3>
            
            {/* [추가] 검색창 및 행사 필터링 드롭다운 그룹 */}
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              ...
              <div className="w-full sm:w-56 relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-50 border-2 border-slate-200 text-slate-700 font-bold rounded-xl outline-none focus:border-blue-500 transition-all"
                  value={searchTerm}
                  onChange={(e) => {
                    setSearchTerm(e.target.value);
                    setSelectedIds([]); // 검색어가 바뀌면 선택된 항목 초기화
                  }}
                />
              </div>
              ...
              <div className="w-full sm:w-56">
                ...
                >
                  <option value="all">모든 행사 보기</option>
                  {events.map(event => (
              ...
              <div className="absolute top-full left-0 w-full mt-2 bg-white border-2 border-slate-200 rounded-2xl shadow-2xl z-50 p-4 max-h-80 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {filteredBooths.length === 0 ? (
                    <div className="col-span-1 md:col-span-2 text-center text-slate-400 font-bold text-sm py-4">해당 조건에 맞는 부스가 없습니다.</div>
                  ) : (
        ...
        <div className="grid gap-4">
          ...
          {filteredBooths.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-3xl border border-slate-200 text-slate-400 font-bold">
              {searchTerm ? "검색 결과가 없습니다." : "등록된 부스가 없습니다."}
            </div>
          ) : (
